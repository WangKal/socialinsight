...
const promoBanners = [
  ...
  "linear-gradient(45deg, #243949 0%, #517fa4 100%)",
...
export default function PublicPromoView() {
  ...
  const copyPromoId = async (promoId?: string) => {
    if (!promoId) return;
    try {
      await navigator.clipboard.writeText(promoId);
    } catch (error) {
      console.error("Failed to copy promo id:", error);
    }
  };
  ...
  const filteredWinners =
    processedPromo?.winners?.filter(
      (winner) =>
        winner.username.toLowerCase().includes(winnerSearchQuery.toLowerCase()) ||
        winner.text.toLowerCase().includes(winnerSearchQuery.toLowerCase())
    ) || [];

  const filteredReplies =
    processedPromo?.replies?.filter(
      (reply) =>
        reply.username.toLowerCase().includes(replySearchQuery.toLowerCase()) ||
  ...
  const filteredPromos = useMemo(() => {
    ...
    return promos.filter((promo: Promo) => {
      const titleMatch = promo.title?.toLowerCase().includes(query);
      const promoIdMatch = (promo.promo_id || "").toLowerCase().includes(query);
      return titleMatch || promoIdMatch;
    });
  }, [promos, promoSearchQuery]);
                ...
                <div className="relative h-[400px]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      ...
                      {processedPromo.promo_id && (
                        <Badge className="bg-black/30 text-white border-white/20 backdrop-blur-sm font-mono">
                          Promo ID: {processedPromo.promo_id}
                        </Badge>
                      )}
                    </div>
            ...
            <div className="grid md:grid-cols-4 gap-4 mb-8">
              ...
              >
                ...
                <p className="text-2xl text-yellow-600">
                  {processedPromo.winners?.length || 0} / {processedPromo.numberOfWinners}
                </p>
              </motion.div>
            </div>
            ...
            {processedPromo.requirementInstructions && (
              ...
              >
                ...
                <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-xl p-6 space-y-6">
                  ...
                  {processedPromo.promo_id && (
                    <div className="border-t border-violet-200 pt-4">
                      <p className="text-sm text-gray-500 mb-2">Promo ID</p>
                      <div className="flex flex-wrap items-center gap-3">
                        <span className="px-4 py-2 bg-white rounded-lg border text-sm font-mono text-gray-800">
                          {processedPromo.promo_id}
                        </span>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                          onClick={() => copyPromoId(processedPromo.promo_id)}
                        >
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Save this promo ID so you can easily refer to or check this promo later.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <motion.div
                  ...
                  className="bg-red-50 border border-red-200 rounded-2xl p-6 mb-8 mt-8"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-700 leading-relaxed">
                        Participants should <strong>never send money or make any payment</strong> to
                        participate in a promo.
                        <br />
                        <br />
                        If a promo requires purchasing a product or service:
                        <br />
                        • Ensure the seller or shop is <strong>legitimate and verified</strong>
                        <br />
                        • Proceed at your own discretion
                        <br />
                        <br />
                        This platform is not responsible for payments, transactions, prize delivery,
                        or interactions related to any promotion.
                      </p>
                    </div>
                  </div>
                </motion.div>
                ...
                <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-6 border-b border-gray-200">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Button
                      ...
                      disabled={!processedPromo.winners || processedPromo.winners.length === 0}
                    >
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <p className="text-gray-900 mb-1">@{winner.username}</p>
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-50 via-white to-purple-50">
      <div className="max-w-7xl mx-auto px-6 py-12">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-5xl font-bold mb-3 bg-gradient-to-r from-violet-600 via-purple-600 to-blue-600 bg-clip-text text-transparent">
            Active Promos
          </h1>
          <p className="text-xl text-gray-600">
            Browse active giveaways and quickly find promos by title or Promo ID
          </p>
        </motion.div>
        ...
        >
          <div className="relative max-w-3xl mx-auto">
            <div className="absolute inset-0 bg-gradient-to-r from-violet-500/15 via-purple-500/10 to-blue-500/15 blur-2xl rounded-[2rem]" />
            <div className="relative bg-white/90 backdrop-blur-xl border border-violet-100 shadow-2xl rounded-[2rem] p-3">
              <div className="relative">
                <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-violet-500" />
                <input
                  type="text"
                  placeholder="Search promos by title or Promo ID..."
                  value={promoSearchQuery}
                  onChange={(e) => setPromoSearchQuery(e.target.value)}
                  className="w-full h-16 pl-14 pr-5 rounded-2xl bg-gradient-to-r from-violet-50 via-white to-blue-50 border-2 border-transparent focus:border-violet-400 focus:ring-4 focus:ring-violet-100 outline-none text-base md:text-lg text-gray-800 placeholder:text-gray-400 transition-all"
                />
              </div>
            </div>
          </div>
          ...
          <div className="text-center mt-3">
            <p className="text-sm text-gray-500">
              Search using a promo title or the unique Promo ID you saved earlier.
            </p>
          </div>
        </motion.div>

        <div className="grid md:grid-cols-2 gap-6">
          {filteredPromos.map((promo: Promo, index: number) => (
            <motion.div
              key={promo.id}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              {promo.brandingImage ? (
                <div className="relative h-64 overflow-hidden group/image">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <p className="text-sm text-white/90 line-clamp-2">{promo.description}</p>
                  </div>
                </div>
              ) : (
              ...
              <div className="p-6">
                {promo.promo_id && (
                  <div className="mb-4 pb-4 border-b border-gray-200">
                    ...
                    <div className="inline-flex items-center rounded-xl bg-gray-100 border border-gray-200 px-3 py-2">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Save this ID to refer to or check this promo later.
                    </p>
                  </div>
                )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {promo.createForHandle && (
                          <p className="text-xs text-gray-500 truncate">{promo.createForHandle}</p>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        {filteredPromos.length === 0 && promoSearchQuery.trim() !== "" && (
          <motion.div
            initial={{ opacity: 0, y: 24 }}
            animate={{ opacity: 1, y: 0 }}
            className="flex items-center justify-center py-16"
          >
            <div className="bg-white border border-gray-200 shadow-xl rounded-3xl px-8 py-12 text-center max-w-xl w-full">
              <Search className="w-12 h-12 text-violet-400 mx-auto mb-4" />
              <h2 className="text-2xl font-bold text-gray-900 mb-2">No matching promos found</h2>
              <p className="text-gray-600">
                Try searching with another title or check that the Promo ID is correct.
              </p>
            </div>
          </motion.div>
        )}
            ...
            <div className="relative max-w-2xl w-full">
              ...
              <div className="relative bg-white/80 backdrop-blur-xl border border-white/70 shadow-2xl rounded-3xl px-8 py-14 text-center overflow-hidden">
                ...
                <p className="text-gray-600 text-base sm:text-lg leading-relaxed max-w-xl mx-auto mb-8">
                  There are no active promos right now, but fresh giveaways, rewards, and
